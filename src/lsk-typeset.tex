...
\verb|\\| 또는 \ci{newline}
\end{lscommand}
\noindent 이 명령은 새로운 문단을 시작하지 않은 상태에서 줄나눔을 행한다.\trfnote{이 안내서의 많은 예제에서 \texttt{\textbackslash\textbackslash}에 의하여 강제 개행하는 예를 보이고 있다. 그러나 그것은 좁은 example 공간에서 부득이하게 그리 한 것이다. 문단 나누기를 위해서는 빈 줄을 두어야 한다. 강제 개행을 문단 나누기 대신 써서는 안 된다.}
...
처리해준다. 한국어(한글)를 이 패키지로 조판하는 문제에 대하여는 \pageref{sec:koreanlang} 페이지
\ref{sec:koreanlang}절을 보라.
...
RTL 언어를 식자하기 위하여 \pai{polyglossia} 패키지는 \pai{bidi} 패키지를 요구한다. %(\pai{bidi}는 \hologo{LuaTeX}을 지원하지 않는다.)\trfnote{2020년 현재 \pai{bidi} 패키지는 \hologo{XeTeX}과 \hologo{LuaTeX}을 모두 잘 지원한다.}
...
\subsection{한글과 한국어 문서}\label{sec:koreanlang}
\index{한국어}\index{Korean}\index{kotex}
\secbynomail{역자가 한국어판을 위해 추가}

\subsubsection{\koTeX{}을 쓰지 않는 경우}
...
그러나 이것만으로는 그저 한글의 인쇄가 가능한 것뿐이고, 특히 한글 문장의
조판에 필요한 요소들이 지원되지 않기 때문에 한국어 문장의 만족스런 조판이 
가능하다고 하기 어렵다. 단지 한두 글자의 한글 문자를 표현할 수 있는 것이라고 
보아야 한다.

\paragraph{\pai{babel}과 \pai{polyglossia}}
\pai{babel}과 \pai{polyglossia}는 다국어 조판을 가능하게 하는
패키지이다. 
\XeTeX{} 또는 Lua\TeX 엔진과 이 다국어 패키지를 통하여
한국어를 한글로 조판할 수 있다. 
다국어 문서에서는 주언어(main language)가 
무엇인가가 매우 중요하다. 영어가 주언어이고 한글로 된 한국어 문장이 
보조적으로 사용되는 경우를 생각하자.

\pai{fontspec}과 \pai{babel}을 이용하여 한국어 언어와 폰트를 선언한다.
\begin{verbatim}
\usepackage{fontspec}
...
\end{verbatim}
이제 한글을 식자하기 위해서 \cmd{\foreignlanguage} 명령
또는 \ei{otherlanguage} 환경을 이용하여야 한다. 
\begin{verbatim}
Hello, \foreignlanguage{korean}{안녕}
\begin{otherlanguage}{korean}
안녕하세요
\end{otherlanguage}
\end{verbatim}

만약 한국어가 주언어인 문서라면 \cmd{\babelprovide}의 옵션에 \texttt{[main]}을
선언할 수 있다. 이 때에는 별도의 명령이나 환경을 사용하지 않고 본문 기본 폰트를 한글 표현이 가능한 것으로 지정하여
한글을 조판할 수 있다.
...
\XeLaTeX 이 이른바 `레거시 텍'과 가장 크게 다른 점은 시스템에 설치된 트루타입과 오픈타입 폰트를 \LaTeX{} 문서에서 사용할 수 있다는 점이다. \koTeX 도 한글 폰트를 자유롭게 사용할 수 있다. \XeLaTeX 에서 한글 문서의 폰트 문제에 대하여 \pageref{kofonts}페이지의 \ref{kofonts}절에 약간의 정보를 추가해두었다. 별다른 설정이 없으면 한글 본문글꼴로 `은 바탕'이 사용된다.
...
타자기를 쓰던 시절에는 중요한 단어에 \underline{밑줄을 그어서} \emph{강조했다}.
\begin{lscommand}
\ci{underline}\verb|{|\emph{text}\verb|}|
\end{lscommand}
